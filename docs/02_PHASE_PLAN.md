...
| **3** | 🔲 대기 | 원격 제어 — 뷰어 입력 → 호스트 enigo 주입 |
| **4** | 🔲 대기 | 파일 전송 + 채팅 + 시스템 트레이 |
| **5** | 🔲 대기 | 보안 강화 + TURN 릴레이 + Electron 패키징 |
...
| 항목 | 현재 상태 | 개선 시점 |
|------|-----------|-----------|
| 비밀번호 비교 | SHA-256 `==` | Phase 5 (Argon2id) |
| 시그널링 암호화 | 없음 (ws://) | Phase 5 (wss://) |
| P2P 실패 처리 | 없음 (STUN만) | Phase 5 (TURN) |
